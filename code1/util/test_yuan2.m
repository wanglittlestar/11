...
%  type = 'denoising';
%      lambda = 5.4;  strImage = 'cameraman';
%  lambda = 9;  strImage = 'barbara';
%     lambda = 8;  strImage = 'lenna';
...
for iii = 1:length(lambdas),
    ...
    type = 'denoising';
    %  type = 'deblurring';
    %  lambda = 0.2;  strImage = 'cameraman';
    %  lambda =0.21;  strImage = 'barbara';
    %  lambda = 0.13;  strImage = 'lenna';
    B_Clean = double(sum(imread('cameraman_cle.png'),3));
    
    %     noisetype = 'Salt-and-Pepper';
    noiselevel = 0.5;
    noisetype = 'Random-Valued';
    
    O = ones(size(B_Clean));
    if(strcmp(noisetype,'Salt-and-Pepper'))
        B_Corrupted = impulsenoise(B_Clean,noiselevel,0);
        max_val = max(abs(B_Corrupted(:))); min_val = min(abs(B_Corrupted(:)));
        O(B_Corrupted==max_val)=0; O(B_Corrupted==min_val)=0;
    elseif(strcmp(noisetype,'Random-Valued'))
        B_Corrupted = impulsenoise(B_Clean,noiselevel,1);
    end
    
    B_Clean = B_Clean/max(B_Clean(:));
    B_Corrupted = B_Corrupted/max(B_Corrupted(:));
    
    %     B_Corrupted = double(sum(imread('cameraman_dir.png'),3));
    p = 2;
    P = GenBlurOper;
    ...
    acc = 1/255;
    kkk = round(numel(B_Corrupted) * rrr);
    % imshow(B_Corrupted)
    % dd
    [U] = l0tv_proj_reg(B_Corrupted,Amap,Atmap,p,kkk,LargestEig,acc,B_Clean);
    %     [U] = l0tv_mpec_reg(B_Corrupted,O,Amap,Atmap,p,lambda,LargestEig,acc,B_Clean);
    %   [U2] = l0tv2(B_Corrupted,Amap,Atmap,p,lambda,LargestEig,acc,B_Clean); PSNR2   =  snr(U2, B_Clean);
    PSNR   =  snr(U, B_Clean);
    psnrs = [psnrs;PSNR];
    %     psnrs2 = [psnrs2;PSNR2];
    psnrs
